...
module Win32cr::System::VirtualDosMachines
  ...
  record VDMLDT_ENTRY,
    limit_low : UInt16,
    base_low : UInt16,
    high_word : HighWord_e__Union do

    # Nested Type HighWord_e__Union
    @[Extern(union: true)]
    record HighWord_e__Union,
      bytes : Bytes_e__Struct,
      bits : Bits_e__Struct do

      # Nested Type Bits_e__Struct
      @[Extern]
      record Bits_e__Struct,
        _bitfield : UInt32


      # Nested Type Bytes_e__Struct
      @[Extern]
      record Bytes_e__Struct,
        base_mid : UInt8,
        flags1 : UInt8,
        flags2 : UInt8,
        base_hi : UInt8
